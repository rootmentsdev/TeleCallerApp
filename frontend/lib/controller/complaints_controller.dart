  ...
  Future<void> fetchComplaints({
    ...
    try {
      ...
      if (response.containsKey('complaints')) {
        ...
        print('ComplaintsController: Found complaints key, type: ${complaintsData.runtimeType}');
        if (complaintsData is List) {
          print('ComplaintsController: Parsing ${complaintsData.length} complaints');
          for (var item in complaintsData) {
            try {
              if (item is Map<String, dynamic>) {
                ...
                print('ComplaintsController: Skipping invalid item (not a Map): $item');
              }
            } catch (e, stackTrace) {
          ...
          print('ComplaintsController: complaints data is not a List: ${complaintsData.runtimeType}');
        }
      } else if (response.containsKey('data')) {
        final complaintsData = response['data'];
        print('ComplaintsController: Found data key, type: ${complaintsData.runtimeType}');
        if (complaintsData is List) {
          print('ComplaintsController: Parsing ${complaintsData.length} complaints from data');
          for (var item in complaintsData) {
            try {
              if (item is Map<String, dynamic>) {
                final complaint = ComplaintModel.fromJson(item);
          ...
          print('ComplaintsController: data is not a List: ${complaintsData.runtimeType}');
        }
      } else {
        print('ComplaintsController: Response does not contain complaints or data keys');
        print('ComplaintsController: Response structure: $response');
        // Check if response has any data that we might have missed
        if (response.isNotEmpty) {
          print('ComplaintsController: Warning: Response has data but no complaints/data key');
          print('ComplaintsController: Response keys: ${response.keys.toList()}');
        }
      }
      ...
      if (parsedComplaints.isEmpty) {
        ...
        final hasComplaintsData = (response.containsKey('complaints') && 
        ...
        if (hasComplaintsData) {
          print('ComplaintsController: Warning: API returned data but parsing failed for all items');
      ...
      print('ComplaintsController: Successfully parsed ${_complaints.length} complaints');
      ...
      if (parsedComplaints.isNotEmpty) {
        ...
        print('ComplaintsController: Sample complaint - ID: ${sample.id}, Name: ${sample.name}');
        print('ComplaintsController: Sample complaint - Remarks: ${sample.remarks}');
        print('ComplaintsController: Sample complaint - Complaint Remarks: ${sample.complaintRemarks}');
        print('ComplaintsController: Sample complaint - Call Status: ${sample.callStatus}');
        print('ComplaintsController: Sample complaint - Call Duration: ${sample.callDuration}');
        print('ComplaintsController: Sample complaint - Has Call Been Made: ${sample.hasCallBeenMade}');
      } else {
